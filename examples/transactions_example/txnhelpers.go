...
// rebalanceCallback is triggered on consumer rebalance.
// For each assigned partition a transactional producer is created, this is required to guarantee
// per-partition offset commit state prior to KIP-447 being supported.
...
func commitTransactionForInputPartition(partition int32) {
	...
	consumerMetadata := processorConsumer.GetConsumerGroupMetadata()
	defer consumerMetadata.Close()
