...
// and emitting the traffic light states to the output topic,
// all using the transactional API.
...
func trafficLightProcessor(wg *sync.WaitGroup, termChan chan bool) {
	...
	// If punctuate is true the current intersection state is
	// written to the output topic every ticker interval regardless if
	// there was a intersection light state change.
